...
# Control snake's length so total length is less than maximum length
def snake_length(p1_queue, max_length):
  # Initiate snake's length
  total_distance = 0
  
  # Define a loop that always take x coordinate as first point
  for i in range(0,len(p1_queue) - 3,2):
    # Define coordinates and their succesive coordinates in snake
    x1 = p1_queue[i]
    y1 = p1_queue[i + 1]
    x2 = p1_queue[i + 2]
    y2 = p1_queue[i + 3]
    
    # Calculate snake's total length
    total_distance =  total_distance + dist(x1, y1, x2, y2)
      
    # Shorten snake's length when snake's total length exceeds maximum length
    while total_distance > max_length:
      # Remove first x and y coordinates from snake's list
      x1 = p1_queue.pop(0)
      y1 = p1_queue.pop(0)
      # State first x and y coordinate after the removal
      x2 = p1_queue[0]
      y2 = p1_queue[1]
      # Calculate new snake's total length
      total_distance =  total_distance - dist(x1, y1, x2, y2)

# Part 5

# Define a function to check if snake's head collides its body
def crash_snake(x1, y1, x2, y2, p1_queue_new):
  # Define a loop that always take x coordinate as first point
  for i in range(0,len(p1_queue_new) - 3,2):
    x3 = p1_queue_new[i]
    y3 = p1_queue_new[i + 1]
    x4 = p1_queue_new[i + 2]
    y4 = p1_queue_new[i + 3]     
  
    # Call another function to check if snake's head line segment intersect any snake's line segment
    if doIntersect(x1, y1, x2, y2, x3, y3, x4, y4) == True:
      return True
...
###############################################################################
    ...
    #
    if p1_lost == False:
      # Calculate snake x and y coordinates
    ...
    #
    if p1_lost == False:
      # Insert x coordinate at the end of snake's list followed by y coordinate
      p1_queue.append(p1_x)
      p1_queue.append(p1_y)
    #
    # Part 3: A Growing Snake
    #
    # Call function to control snake's length 
    if p1_lost == False:    
      snake_length(p1_queue, max_length)
    #
    ...
    # Define boundary at the left and right sides
    if p1_x < MIN_WIDTH or p1_x > MAX_WIDTH and p1_y >= MIN_HEIGHT and p1_y <= MAX_HEIGHT:
      # Snake stops and turns red if it hits the boundary
      p1_lost = True
      
    # Define boundary at the top and bottom sides
    if p1_y < MIN_HEIGHT or p1_y > MAX_HEIGHT and p1_x >= MIN_WIDTH and p1_x <= MAX_WIDTH:
      # Snake stops and turns red if it hits the boundary
      p1_lost = True
    #
    # Part 5: Colliding with Yourself
    #
    # Define the snake's head line segment as coordinates
    x1 = p1_queue[len(p1_queue) - 4]
    y1 = p1_queue[len(p1_queue) - 3]
    x2 = p1_queue[len(p1_queue) - 2]
    y2 = p1_queue[len(p1_queue) - 1]
    
    # New list for snake's coordinates that excludes the two most recently added segment
    p1_queue_new = p1_queue[0:len(p1_queue) - 5]
     
    # Snake stops and turns red if it hits itself
    if crash_snake(x1, y1, x2, y2, p1_queue_new) == True:
      p1_lost = True
    #
    # Part 6: Colliding with Other Snakes
    #
    # Define a loop that depends on the number of AI snakes chosen
    for other_snakes in e_queues:
      # Snake stops and turns red if it hits AI snakes        
      if crash_snake(x1, y1, x2, y2, other_snakes) == True:
        p1_lost = True
